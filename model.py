    ...
    def call(self, inputs, state):
        ...
        for l, layer in enumerate(self._cells):
            ...
            if l > 0:
                ...
                layer_input = tf.compat.v1.concat((layer_input[:, -offset_width:], layer_input[:, :-offset_width]),
                                                  axis=1, name='concat_offset_%d' % l)
            # Create a tuple of inputs by splitting the lower layer output into partitions.
            p_inputs = tf.compat.v1.split(layer_input, len(layer), axis=1, name='split_%d' % l)
            p_outputs = []
            p_states = []
            ...
            layer_input = tf.compat.v1.concat(p_outputs, axis=1, name='concat_%d' % l)
...
def _rnn_state_placeholders(state):
    ...
    # if isinstance(state, tf.compat.v1.contrib.rnn.LSTMStateTuple):
    #     c, h = state
    #     c = tf.compat.v1.placeholder(c.dtype, c.shape, c.op.name)
    #     h = tf.compat.v1.placeholder(h.dtype, h.shape, h.op.name)
    #     return tf.compat.v1.contrib.rnn.LSTMStateTuple(c, h)
    if isinstance(state, tf.compat.v1.Tensor):
        h = state
        h = tf.compat.v1.placeholder(h.dtype, h.shape, h.op.name)
        return h
    else:
...
class Model():
    def __init__(self, args, infer=False):  # infer is set to true during sampling.
        ...
        self.lr = tf.compat.v1.Variable(args.learning_rate, name="learning_rate", trainable=False)
        self.global_epoch_fraction = tf.compat.v1.Variable(0.0, name="global_epoch_fraction", trainable=False)
        self.global_seconds_elapsed = tf.compat.v1.Variable(0.0, name="global_seconds_elapsed", trainable=False)
        ...
        self.input_data = tf.compat.v1.placeholder(tf.compat.v1.int32, [args.batch_size, args.seq_length])

        self.zero_state = cell.zero_state(args.batch_size, tf.compat.v1.float32)
        ...
        with tf.compat.v1.variable_scope('rnnlm'):
            ...
            # Create an embedding tensor with tf.compat.v1.nn.embedding_lookup(embedding, self.input_data).
        ...
        output = tf.compat.v1.reshape(outputs, [-1, layer_size])
        ...
        self.logits = tf.compat.v1.matmul(output, softmax_w) + softmax_b
        if infer:
            ...
            self.probs = tf.compat.v1.nn.softmax(self.logits)
        else:
            ...
            self.targets = tf.compat.v1.placeholder(tf.compat.v1.int32, [args.batch_size, args.seq_length])
            ...
            loss = nn_ops.sparse_softmax_cross_entropy_with_logits(
                labels=tf.compat.v1.reshape(self.targets, [-1]), logits=self.logits)
            ...
            self.cost = tf.compat.v1.reduce_mean(loss)
            # Create a tensorboard summary of our cost.
            tf.compat.v1.summary.scalar("cost", self.cost)

            tvars = tf.compat.v1.trainable_variables()  # tvars is a python list of all trainable TF Variable objects.
            # tf.compat.v1.gradients returns a list of tensors of length len(tvars) where each tensor is sum(dy/dx).
            grads, _ = tf.compat.v1.clip_by_global_norm(tf.compat.v1.gradients(self.cost, tvars),
                                                        args.grad_clip)
            optimizer = tf.compat.v1.train.AdamOptimizer(self.lr)  # Use ADAM optimizer.
    ...
    def trainable_parameter_count(self):
        total_parameters = 0
        for variable in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope='rnnlm'):
